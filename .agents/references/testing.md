...
- `.\gradlew.bat jacocoAggregateReport` for the merged unit and release-matrix UI coverage report. It aggregates `build/jacoco/test.exec` with `build/jacoco/releaseMatrixUiTest.exec` when present, so coverage for the IntelliJ platform adapter classes that unit tests cannot reach is captured from the Starter-launched IDE. The release-matrix UI lane attaches the JaCoCo agent to the IDE process and uploads this report to Codecov under the `integration` flag; pass `-Paicommitall.integrationCoverage=false` to skip the agent if it ever destabilizes the lane. The unit `verifyJacocoCoverageReport` gate is intentionally left independent of this report so pull-request CI does not depend on the heavy UI lane.

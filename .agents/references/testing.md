...
- `.\gradlew.bat jacocoAggregateReport` for the combined unit and release-matrix UI coverage report. It merges the unit `test.exec` with the same job's release-matrix `releaseMatrixUiTest.exec` against the instrumented production classes, so coverage for the IntelliJ platform adapter classes that unit tests cannot reach is captured from the Starter-launched IDE. Each release-matrix UI matrix job attaches the JaCoCo agent to its IDE process, then builds this report against its own up-to-date instrumented classes (the exact bytecode the IDE loaded) and uploads it to Codecov under the `aggregate` flag; Codecov merges the per-IDE-product uploads. Building one report from a separate rebuild would discard the integration probes (JaCoCo class-id mismatch), which is why each product reports against its own build rather than a downstream merge job. Pass `-Paicommitall.integrationCoverage=false` to skip the agent if it ever destabilizes the lane. The unit `verifyJacocoCoverageReport` gate is intentionally left independent of this report so pull-request CI does not depend on the heavy UI lane.

...
- Use one long-running worker for the entire plan.
    - Why it was not chosen: accumulated context can carry stale assumptions across tasks and blur task-specific validation evidence.
- Spawn multiple workers in parallel by default.
    - Why it was not chosen: most plan tasks in this repository can touch shared Gradle, plugin descriptor, UI, or documentation files, so default parallel work risks conflicts.
- Keep all work in the orchestrator even when delegation is available.
    - Why it was not chosen: fresh task workers can keep implementation context smaller while the orchestrator preserves plan continuity.

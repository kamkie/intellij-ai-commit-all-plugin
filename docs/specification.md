...
- REQ-ERR-003: For workflow stop reasons not covered by a standard platform message, the plugin MAY surface a plugin-owned warning notification through the `AI Commit All` group. The plugin-owned notification bodies are the AI timeout text: `AI Assistant did not finish generating a commit message before the configured timeout.` and the staging-confirmation failure text reported for `StagingConfirmationFailed`: `Staging could not be confirmed for the selected changes. Refresh the Commit tool window or repeat the action; the staging area state may be stale.` Source: ADR 0014, ADR 0088. Implements: T-ERROR-004, T-ERROR-007. Validates: SCN-WORKFLOW-*, SCN-STAGE-AUT-*.
...
| ADR 0088 | REQ-ACT-006, REQ-ERR-003, REQ-ERR-004                                                                                |

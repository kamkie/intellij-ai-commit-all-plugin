...
- REQ-ACT-006: When commit workflow activation or synchronization initially cannot prove a supported workflow, the plugin MUST retry within a bounded settling window before reporting `UnsupportedWorkflow`, except that a staging-state confirmation failure after its bounded window MUST report `StagingConfirmationFailed` per REQ-ERR-003. A workflow type that is definitively unsupported remains terminal and MUST stop immediately without AI, commit, or push work. Source: ADR 0084, ADR 0088. Validates: SCN-WORKFLOW-*.
...
- REQ-SEL-008: The workflow MUST NOT invoke AI generation until staging state confirms that every expected path is staged or satisfied. Direct per-path Git index confirmation after staging MUST take precedence over blocking IDE status and tracker refresh waits when every expected path is either staged or HEAD-identical. A HEAD-identical path contributes no diff and need not appear in AI input. Any unconfirmed path or Git index check failure MUST retain the bounded IDE refresh, retry, and fail-closed fallback. Before invoking AI, the same EDT turn MUST apply the confirmed tracker state and included Git roots to the Commit UI and MUST verify through the current `CommitWorkflowUi` inclusion model that every expected staged path is visible; AI MUST NOT be invoked when that verification fails. The default Commit workflow MUST retain that exact confirmed handoff through AI generation and MUST reapply and verify it immediately before invoking the IDE default commit executor so later tracker refreshes cannot clear the intended commit roots. Later visual refreshes MAY remain asynchronous and best effort, but MUST NOT be required for either handoff. Source: PLAN-confirm-staged-before-ai-generation, PLAN-fast-path-staging-confirmation, PLAN-workflow-stop-feedback-and-push-alignment, PLAN-staging-enabled-commit-ui-reliability, T-BUG-019, T-BUG-020. Validates: SCN-STAGE-AUT-007..014, SCN-STAGE-AUT-020, SCN-STAGE-AUT-027.
...
- REQ-AI-012: When AI generation completes but the resulting message is empty, the workflow MUST stop and MUST report `EmptyMessage`. The stop MUST surface the plugin-owned empty AI message warning per REQ-ERR-003 instead of the standard empty-commit-message platform text. Source: ADR 0014, ADR 0088. Validates: SCN-AI-*, SCN-STAGE-MAN-013.
...
- REQ-PUSH-002: Safe immediate push MUST be used, skipping the IDE Push Commits dialog, only when ALL of the following are verified for every affected Git repository: the current branch has a tracked upstream; the target push is unambiguous; the target is the tracked upstream; the target is not a new branch or special ref; the repository state is `NORMAL`; and no unresolved conflicts are present in the affected commit scope. Source: ADR 0047, ADR 0087. Validates: SCN-PUSH-*.
- REQ-PUSH-003: `Push` MUST NOT require local-matches-upstream verification on either the commit-and-push or outgoing-only path, allowing an already-ahead local branch to push. Source: ADR 0047, alpha.9 fix, ADR 0087. Validates: SCN-PUSH-*.
...
- REQ-ERR-002: For workflow stop reasons covered by standard IntelliJ platform messages, the plugin MUST surface the standard platform text rather than custom text. Currently this applies to `EmptySelection` (`error.no.changes.to.commit`). `EmptyMessage` is no longer covered by a standard platform text; it reports the plugin-owned empty AI message warning per REQ-ERR-003. Source: ADR 0016, ADR 0088. Implements: T-ERROR-001, T-ERROR-005. Validates: SCN-WORKFLOW-*.
- REQ-ERR-003: For workflow stop reasons not covered by a standard platform message, the plugin MAY surface a plugin-owned warning notification through the `AI Commit All` group. The plugin-owned notification bodies are the AI timeout text: `AI Assistant did not finish generating a commit message before the configured timeout.`, the staging-confirmation failure text reported for `StagingConfirmationFailed`: `Staging could not be confirmed for the selected changes. Refresh the Commit tool window or repeat the action; the staging area state may be stale.`, and the empty AI message text reported for `EmptyMessage`: `AI Assistant finished without producing a commit message. Repeat the action; very large changes are a known trigger for AI generation giving up.` Source: ADR 0014, ADR 0088. Implements: T-ERROR-004, T-ERROR-007. Validates: SCN-WORKFLOW-*, SCN-STAGE-AUT-*, SCN-AI-*.
- REQ-ERR-004: Workflow stop reasons `MissingWorkflow`, `UnsupportedVcs`, `UnsupportedWorkflow`, `MissingAiAction`, `AiCompletionFailed`, `UnchangedMessage`, `NoCompletionSignal`, `UserEditedMessage`, `CommitExecutionUnavailable`, `PushExecutionUnavailable`, `VcsFrozen`, and `VcsBackgroundOperationRunning` MUST NOT trigger plugin-owned notifications; their user-visible effect MUST be limited to existing IDE-owned UI or to no notification when the IDE has none for that state. A staging-state confirmation failure stops with `StagingConfirmationFailed` (governed by REQ-ERR-003), so `UnsupportedWorkflow` remains silent and covers only genuinely unsupported workflow types. Source: ADR 0016, ADR 0088. Validates: SCN-WORKFLOW-*.
...
| `StagingConfirmationFailed`     | Git staging-state confirmation failed after the bounded staging-confirmation window. | Final after bounded staging-confirmation settling.                       |

...
This repository has no Marketplace-published plugin version yet. `v0.1.0-beta.8` is the current implementation prerelease candidate prepared from this repository.
...
- Avoid intermittent first-click no-ops on the `Push` section when the toolbar component's action-time data context changes during one click.

## [v0.1.0-beta.7] - 2026-06-25

### Fixed

- Avoid waiting on a synchronous Commit UI visual refresh after Git index-confirmed staging; the workflow now updates the required staging model state, schedules the visual tracker/root refresh asynchronously, and logs each synchronization substep.

## [v0.1.0-beta.6] - 2026-06-24

### Fixed

- Continue immediately after Git index confirmation with an index-confirmed Commit UI state when the IDE staging tracker remains stale, avoiding focus-dependent hangs after background staging has already succeeded.
...
- Retry transiently missing push metadata within the bounded settling window when the local branch is ahead of its tracked upstream, on both the commit-and-push and outgoing-only push paths, instead of falling back to the IDE Push Commits dialog or stopping immediately.

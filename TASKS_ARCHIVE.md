...
- [x] T-REL-018: make version number be sortable. Marketplace/plugin versions now drop the tag-only leading `v` and use SemVer build metadata for post-tag or dirty builds, while local distribution ZIP versions use the same readable metadata without the `g` prefix before Git hashes. (`build.gradle.kts`, `src/test/kotlin/pl/devopssolutions/aicommitall/ci/GitHubActionsWorkflowTest.kt`, `CHANGELOG.md`; validation: red `.\gradlew.bat test --tests "pl.devopssolutions.aicommitall.ci.GitHubActionsWorkflowTest.gradle versioning keeps marketplace semver and readable local zip metadata"` before implementation; green same command, `.\gradlew.bat verifyPluginVersionFormatting`, `.\gradlew.bat buildPlugin`)

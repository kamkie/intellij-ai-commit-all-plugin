...
- Before broad exploration or edits, check thread size and compaction risk. When the user request authorizes subagents, delegation, or parallel agent work, use delegated workers or read-only sidecars to avoid context compaction; otherwise keep context narrow and warn if compaction risk is high.
